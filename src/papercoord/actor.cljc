(ns papercoord.actor
  "PaperCoordActor — the ISCO-08 8143 paper products machine operators
  ...
  The unconditional invariant: the Paper Products Plant Scheduling
  Coordination Advisor can never directly commit an operating record
  the PaperCoordGovernor refuses, and can never make this actor
  finalize a machine-operation-execution decision (e.g. deciding to
  proceed with a specific converting/cutting/folding run) or a
  plant-safety-clearance decision (e.g. declaring the plant safety
  cleared), and can never override a plant safety officer's judgment —
  ...
  never operates converting equipment itself, and it never makes a
...
(defn build-graph
  "Build a compiled PaperCoordActor graph. `store` implements
  `papercoord.store/Store`. `advisor` implements
  `papercoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:converter-id (:converter-id request)
                                    :op (:op proposal)
